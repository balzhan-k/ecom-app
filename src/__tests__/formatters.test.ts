...
describe("formatters utils", () => {
  ...
  test("formatDimensions", () => {
    expect(formatDimensions({ width: 10, height: 20, depth: 30 })).toBe(
      "W: 10cm, H: 20cm, D: 30cm"
    );
    expect(formatDimensions({ width: 15 })).toBe("W: 15cm, H: N/Acm, D: N/Acm");
    expect(formatDimensions(undefined)).toBe("N/A");

    expect(formatDimensions({ height: 40, depth: 50 })).toBe("N/A");
  });
  ...
  test("getProductDisplayTitle", () => {
    expect(getProductDisplayTitle({ brand: "Apple", title: "iPhone" })).toBe(
      "Apple. iPhone"
    );
  });
